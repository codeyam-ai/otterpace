...
public struct SettingsView: View {
    ...
    // connect/disconnect; `coachActive` is the provider Buddy currently uses.
    private let coachKeys = CoachKeyStore()
    @State private var coachConnected: [CoachProvider] = []
    @State private var coachActive: CoachProvider?
    @State private var coachKeyDraft = ""
    /// Set when a pasted key matched no known provider — the card then asks the
    /// user which provider it belongs to instead of rejecting it.
    @State private var coachKeyUnrecognized = false
    ...
    /// Re-read the store into view state. Called on appear and after every
    /// connect/disconnect/switch so the card reflects the Keychain, not a guess.
    private func refreshCoachConnection() {
        coachConnected = coachKeys.connectedProviders
        coachActive = coachKeys.activeProvider
    }

    @ViewBuilder private var coachCard: some View {
        card("AI Coach") {
            VStack(alignment: .leading, spacing: 10) {
                if coachConnected.isEmpty {
                    Text("Connect your own API key from Anthropic, OpenAI, or Gemini for real AI coaching. Without one, Buddy still coaches you with built-in guidance. Your key is stored only on this device and sent over HTTPS to power replies. It's never saved on a server.")
                        .font(Typography.callout).foregroundColor(Palette.ink)
                        .fixedSize(horizontal: false, vertical: true)
                } else {
                    // One row per connected provider. With several connected, the
                    // rows are tappable to switch; with one, it is just a status.
                    VStack(spacing: 0) {
                        ForEach(Array(coachConnected.enumerated()), id: \.element) { idx, provider in
                            if idx > 0 { Divider().opacity(0.25) }
                            CoachProviderRow(
                                provider: provider,
                                isActive: coachActive == provider,
                                canSwitch: coachConnected.count > 1,
                                onActivate: { switchCoachProvider(to: provider) },
                                onDisconnect: { disconnectCoachProvider(provider) }
                            )
                        }
                    }
                    if coachConnected.count > 1 {
                        Text("Tap a provider to switch which one Buddy uses.")
                            .font(Typography.caption).foregroundColor(Palette.subtle)
                            .fixedSize(horizontal: false, vertical: true)
                    }
                }

                // The connect field stays available while providers are connected,
                // so adding a second or third key is one paste, not a disconnect
                // first. It disappears only once all three are connected.
                if coachConnected.count < CoachProvider.allCases.count {
                    CoachKeyField(
                        draft: $coachKeyDraft,
                        unrecognized: $coachKeyUnrecognized,
    ...
    private func switchCoachProvider(to provider: CoachProvider) {
        coachKeys.setActive(provider)
        refreshCoachConnection()
        Analytics.shared.capture("coach_provider_switched", ["provider": provider.rawValue])
    }

    /// Disconnect one provider. The store promotes another connected provider if
    /// this was the active one, so the coach doesn't silently go offline.
    private func disconnectCoachProvider(_ provider: CoachProvider) {
        coachKeys.clear(provider)
        refreshCoachConnection()
        Analytics.shared.capture("coach_provider_disconnected", ["provider": provider.rawValue])
    }

    private func connectCoachKey(as provider: CoachProvider) {
        let key = coachKeyDraft.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !key.isEmpty else { return }
        coachKeys.save(key, for: provider)
        coachKeyDraft = ""
        coachKeyUnrecognized = false
        refreshCoachConnection()
        Analytics.shared.capture("coach_provider_connected", ["provider": provider.rawValue])
    }
    ...
    // Thin wrappers over the shared row components (see SettingsActionRow.swift),
    // kept so the other cards in this file read exactly as they did before.
    private func actionRow(_ title: String, icon: String, tint: Color, destructive: Bool = false, action: @escaping () -> Void) -> some View {
        SettingsActionRow(title: title, icon: icon, tint: tint, destructive: destructive, action: action)
    }

    private func actionRowLabel(_ title: String, icon: String, tint: Color, destructive: Bool = false, external: Bool = false) -> some View {
        SettingsActionRowLabel(title: title, icon: icon, tint: tint, destructive: destructive, external: external)
    }
