...
struct OnboardingFlowView: View {
    ...
    /// Which provider the connected key belongs to, so the confirmation names the
    /// right model family instead of always saying Claude.
    @State private var keyProvider: CoachProvider?
    ...
    init(onFinish: @escaping () -> Void = {},
        ...
        // `CoachKeyStore` honors the scenario seed hooks itself, so this covers
        // both a real connection and a captured one.
        let keys = CoachKeyStore(defaults: defaults)
        _keyConnected = State(initialValue: keys.isConnected)
        _keyProvider = State(initialValue: keys.activeProvider)
    }
    ...
    private var aiKeyStep: some View {
        stepScaffold(
            ...
            subtitle: keyConnected
                ? "You're connected. Replies are generated by \(keyProvider?.modelFamily ?? "your provider"), using your key."
                : "Connect your own API key from Anthropic, OpenAI, or Gemini for real AI coaching. Without one, Buddy still coaches you with built-in guidance. Your key is stored only on this device."
        ) {
            if keyConnected {
                HStack(spacing: 10) {
                ...
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    // Name the provider as soon as the shape is recognizable, so
                    // the user sees their key landed somewhere sensible.
                    if let detected = CoachProvider.detect(fromKey: keyDraft) {
                    ...
                    let linkProvider = CoachProvider.detect(fromKey: keyDraft) ?? .anthropic
                    if let url = linkProvider.consoleURL {
                        Link(destination: url) {
                            HStack(spacing: 6) {
                                Image(systemName: "key")
                                Text("Get \(linkProvider.article) \(linkProvider.displayName) API key")
            ...
            if keyConnected {
                ...
                primaryButton(hasKey ? "Connect & finish" : "Get started",
                              enabled: true) {
                    // Onboarding stays a single frictionless field: an
                    // unrecognized key is filed under Anthropic (the original
                    // default) rather than blocking the last step with a picker.
                    // Settings is where a mis-filed key can be corrected.
                    if hasKey {
                        let provider = CoachProvider.detect(fromKey: keyDraft) ?? .anthropic
                        coachKeys.save(keyDraft, for: provider)
                        Analytics.shared.capture("onboarding_key_connected", ["provider": provider.rawValue])
                    }
                    finish()
                }
            }
